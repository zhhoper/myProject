...
from defineNormal_direct import *
...
def networkForward(data, Testing=False):
    ...
    # predict normal
    output_normal = my_network(inputs_normal, coarse_normal)
    diff_normal = output_normal - coarse_normal
    ...
    loss['loss_list'] = [loss_normal.data[0], loss_normal_grad.data[0]]

    return loss, output_normal, coarse_normal, diff_normal
...
def main(savePath, load_path, lr=1e-3, weight_decay=0, total_epoch=100):
    ...
    for epoch in range(total_epoch):
        ...
        for i , data in enumerate(trainLoader, 0):
            begin_time = time.time()
            optimizer.zero_grad()
            loss_miniBatch, output_normal, coarse_normal, diff_normal \
                    = networkForward(data, Testing=False)
            begin_time= time.time()
            loss = loss_miniBatch['normal'] + loss_miniBatch['normal_grad']
            loss.backward()
            optimizer.step()
            ...
            if i%monitor_count == monitor_count - 1:
                ...
                print '%.4f '*2 % tuple(tmp_loss_list)
                print>>fid, '%d %5d ' % (epoch+1, i+1),
                print>>fid, '%.4f '*2 % tuple(tmp_loss_list)
        ...
        print >> fid_sep, '%0.6f '* 2 % tuple(loss_list)
        print '%0.6f '* 2 % tuple(loss_list)
        # ---------------------------------------------------------
        # validation 
        # ---------------------------------------------------------
        ...
        print '%.4f '*2 % tuple(test_loss_list)
        print>>fid_test, '%d ' % (epoch+1),
        print>>fid_test, '%.4f '*2 % tuple(test_loss_list)
        ...
        print >> fid_test_sep, '%0.6f '* 2 % tuple(test_loss_list)
        print '%0.6f '* 2 % tuple(test_loss_list)
