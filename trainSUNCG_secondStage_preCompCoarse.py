'''
    second stage with precomputed coarse image
...
coarseFolder = '/scratch1/intrinsicImage/myProject/cvpr2019_code/result/result_coarse_0.0010_0.00_0100/coarse_image'
...
transformer = transforms.Compose([testTransfer(output_size=IMAGE_SIZE), ToTensor()])
...
def networkForward(data, Testing=False):
    ...
    inputs, albedo, shading, normal, mask, \
        ...
        Variable(coarseAlbedo.cuda(), volatile=Testing).float(), \
        Variable(coarseShading.cuda(), volatile=Testing).float(), \
        Variable(coarseNormal.cuda(), volatile=Testing).float(), \
        Variable(coarseLighting.cuda(), volatile=Testing).float()

    # --------------------------------------------------------------------------------
    # get albedo, shading, normal, lighting in coarse scale and prepare the residual
    coarse_albedo = F.upsample(coarseAlbedo, size=[IMAGE_SIZE, IMAGE_SIZE], mode='bilinear')
    coarse_normal = F.upsample(coarseNormal, size=[IMAGE_SIZE, IMAGE_SIZE], mode='bilinear')
    coarse_shading = F.upsample(coarseShading, size=[IMAGE_SIZE, IMAGE_SIZE], mode='bilinear')

    # NOTE: we have a bug in coarse network for lighting, correct it
    coarse_lighting = Variable(coarseLighting[0:27].data).float()
    
    # get the residual as groundtruth
    albedo = Variable((albedo - coarse_albedo).data).float()
    # NOTE: shading should not be the residual
    ...
    return loss, output_albedo + coarse_albedo, output_shading + coarse_shading, \
            output_normal + coarse_normal
    ...
    for epoch in range(total_epoch):
        ...
        for i , data in enumerate(trainLoader, 0):
            ...
            if i%monitor_count == monitor_count - 1:
                ...
                tmp_loss_list = np.mean(np.array(tmp_loss_list), axis=0)
                print '[%d %5d] loss: ' % (epoch + 1, i+1),
                print '%.4f '*8 % tuple(tmp_loss_list)
                print>>fid, '%d %5d ' % (epoch+1, i+1),
                print>>fid, '%.4f '*8 % tuple(tmp_loss_list)
        ...
        print '%.4f '*8 % tuple(test_loss_list)
        print>>fid_test, '%d ' % (epoch+1),
        print>>fid_test, '%.4f '*8 % tuple(test_loss_list)
