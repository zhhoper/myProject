...
shadingLayer_fine = helpDRN_patched.constructShading(
...
def main(savePath, load_path, lr=1e-3, weight_decay=0, total_epoch=100):
    ...
    for epoch in range(total_epoch):
        ...
        for i , data in enumerate(trainLoader, 0):
            ...
            numLosses = len(loss_miniBatch['loss_list'])
            ...
            if i%monitor_count == monitor_count - 1:
                ...
                print '%.4f '*numLosses % tuple(tmp_loss_list)
                print>>fid, '%d %5d ' % (epoch+1, i+1),
                print>>fid, '%.4f '*numLosses % tuple(tmp_loss_list)
        ...
        print >> fid_sep, '%0.6f '* numLosses % tuple(loss_list)
        print '%0.6f '* numLosses % tuple(loss_list)
        # ---------------------------------------------------------
        # validation 
        # ---------------------------------------------------------
        ...
        for i, data in enumerate(testLoader, 0):
            loss_miniBatch, output_albedo, output_normal, output_shaing,\
        ...
        print '%.4f '*numLosses % tuple(test_loss_list)
        print>>fid_test, '%d ' % (epoch+1),
        print>>fid_test, '%.4f '*numLosses % tuple(test_loss_list)
        ...
        writer.add_image('test_ormal_diff', diff_normal)
        writer.add_image('test_normal_coarse', coarse_normal)
    
        print >> fid_test_sep, '%0.6f '* numLosses % tuple(test_loss_list)
        print '%0.6f '* numLosses % tuple(test_loss_list)
