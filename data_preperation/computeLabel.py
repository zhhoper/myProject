...
def computeLabel(refImg, pointList):
    ...
    boundary_1 = 1 + delta + margin
    boundary_2 = 1 + delta - margin

    numHeight, numWidth, _ = refImg.shape
    meanRef = np.mean(refImg, axis=2).ravel()
    ...
    #print np.sum(ind_lighter)
    #print np.sum(ind_darker)
    #print np.sum(ind_equal)
...
def recordLabel(refImage, indexImg, maskImg, saveName):
    ...
    '''
    numHeight, numWidth,_ = refImage.shape
