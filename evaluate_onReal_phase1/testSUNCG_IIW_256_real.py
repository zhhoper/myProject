...
def main(savePath, coarseModelPath, fineModelPath, detailModelPath):
    ...
    my_network_detail.load_state_dict(torch.load(os.path.join(detailModelPath, 'trained_model', 'trained_model_05.t7')))
    my_network_detail.cuda()
    my_network_detail.train(False)
    ...
    for mode in range(0,3):
        ...
        for ii, data in enumerate(dataset_iiw_test, 0):
            ...
            # get shading
            output_shading = shadingLayer(F.normalize(fine_normal + output_normal, p=2, dim=1), 
                    true_lighting)
            
            output_albedo = output_albedo + fine_albedo
            output_normal = F.normalize((output_normal + fine_normal), p=2, dim=1)
