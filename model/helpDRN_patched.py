...
class constructShading(nn.Module):
    def __init__(self, imageSize, patchSize, stride):
        ...
        self.weight = torch.ones(1, 3, self.imageSize, self.imageSize)
        self.weight[:, :, self.stride:-self.stride, 0:self.stride] = 0.5
        self.weight[:, :, self.stride:-self.stride, -self.stride:] = 0.5
        self.weight[:, :, 0:self.stride, self.stride:-self.stride] = 0.5
        self.weight[:, :, -self.stride:, self.stride:-self.stride] = 0.5
        self.weight[:, :, self.stride:-self.stride, self.stride:-self.stride] = 0.25
        self.weight = Variable(self.weight.cuda()).float()
    ...
    def forward(self, light_UL, light_UR, light_DL, light_DR, normal):
        ...
        shading = (tmp_UL + tmp_UR + tmp_DL + tmp_DR)*self.weight.repeat(self.numImages, 1, 1, 1)
        return shading
