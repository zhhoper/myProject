...
from defineHourglass_noSkip_128 import *
...
def networkForward(data, Testing=False):
    ...
    return loss, output_albedo + coarse_albedo, output_normal + coarse_normal, \
            output_shading, output_normal, coarse_normal
...
def main(savePath, load_path, lr=1e-3, weight_decay=0, total_epoch=100):
    ...
    for epoch in range(total_epoch):
        ...
        for i , data in enumerate(trainLoader, 0):
            begin_time = time.time()
            optimizer.zero_grad()
            loss_miniBatch, output_albedo, output_normal, output_shading, residual_normal, coarse_normal \
            ...
            if i%monitor_count == monitor_count - 1:
                ...
                writer.add_image('residual_normal', residual_normal)
                writer.add_image('coarse_normal', coarse_normal)
