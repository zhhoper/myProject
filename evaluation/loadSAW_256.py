...
def decompose(inputs, my_network_coarse, my_network_fine, my_network_detail, shadingLayer_coarse, shadingLayer):
    ...
    output_albedo, output_normal, output_lighting = my_network_detail(inputs_albedo, inputs_normal, inputs_lighting)
    ...
    # get shading
    output_shading = shadingLayer(F.normalize(fine_normal + output_normal, p=2, dim=1), true_lighting) 
    output_albedo = output_albedo + fine_albedo
...
def get_precision_recall_list_new(pixel_labels_dir, thres_list, photo_ids,
    ...
    for photo_id in (photo_ids):
        ...
        saw_img = cv2.resize(saw_img, (IMAGE_SIZE, IMAGE_SIZE), interpolation=cv2.INTER_CUBIC)
        ...
        prediction_R, prediction_S = decompose(input_images, network[0], network[1], network[2], network[3], network[4])
        #---------------------------------------------------------
