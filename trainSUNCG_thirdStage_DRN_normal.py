...
def networkForward(data, my_network_coarse, my_network_fine, Testing=False):
    '''
    ...
    output_albedo, output_normal, output_lighting = my_network_fine(inputs_albedo, inputs_normal, inputs_lighting)
...
def main(savePath, load_path_coarse, load_path_fine, lr=1e-3, weight_decay=0, total_epoch=100):
    ...
    global my_network_coarse 
    my_network_coarse = torch.load(os.path.join(load_path_coarse, 'trained_model.t7'))
    my_network_coarse.cuda()
    my_network_coarse.train(False)

    global my_netowrk_fine
    my_network_fine = torch.load(os.path.join(load_path_fine, 'trained_model.t7'))
    my_network_fine.cuda()
    my_network_fine.train(False)

    # continue training
    global my_network
    my_network = torch.load('result/result_thirdStage_DRN_normal_0.0010_0.00_0010/trained_model.t7')
    my_network.cuda()
    ...
    for epoch in range(total_epoch):
        ...
        for i , data in enumerate(trainLoader, 0):
            ...
            loss_miniBatch, output_albedo, output_normal, output_shading, \
                   diff_normal, coarse_normal = networkForward(data, my_network_coarse, my_network_fine, Testing=False)
        ...
        for i, data in enumerate(testLoader, 0):
            loss_miniBatch, output_albedo, output_normal, output_shading, \
                    diff_normal, coarse_normal= networkForward(data, my_network_coarse, my_network_fine, Testing=True)
            loss = loss_miniBatch['albedo'] + loss_miniBatch['albedo_grad'] + \
                    loss_miniBatch['shading'] + loss_miniBatch['shading_grad'] + \
                    loss_miniBatch['normal'] + loss_miniBatch['normal_grad']
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        ...
        main(savePath, load_path_coarse, load_path_fine, lr, weight_decay, total_epoch)
    else:
        main(savePath)
